...
interface AnimatedTextProps {
  text: string;
  fontSize?: number; // Peut être un rem ou un vw pour la responsivité
  fill?: string;
  delay: number;
}
    ...
    <div className="flex space-x-2"> {/* Ajouter de l'espace entre les mots */}
      {words.map((word, index) => (
        <motion.div
          ...
          style={{ fontSize: `${fontSize}rem`, color: fill }} // Utiliser rem pour la responsivité
          variants={drawText}
          custom={delay + index * 0.4} // Ajuster le délai pour chaque mot
          initial="hidden"
          animate="visible"
        >
          {word}
